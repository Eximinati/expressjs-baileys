...
const initWaServer = (stateId: string): Promise<void> => {
    return new Promise(async (resolve) => {
        ...
        waServiceClass[stateId].on(`service.whatsapp.qr`, async (value) => {
            ...
            await writeFileSync(
                `${qrCodeBasedir}/qr-code-${waServiceClass[stateId].getCredId()}.txt`,
                value.qr.toString()
            );
        });
...
const runExpressServer = async () => {
    ...
    app.get('/get-qrcode', async (req, res) => {
        ...
        try {
            qrCodeString = await readFileSync(
                `${qrCodeBasedir}/qr-code-${waServiceClass[stateId].getCredId()}.txt`,
                'utf-8'
            );
        } catch (err) {
    ...
    app.post(
        ...
        async (req, res) => {
            ...
            const phoneNumber = isValidPhoneNumber(req.body.phone_number, 'ID')
                ? parsePhoneNumber(req.body.phone_number, 'ID')
                : null;
            if (phoneNumber) {
            ...
            try {
                ...
                await waServiceClass[stateId].sendTextMessage(
                    req.body.phone_number,
                    replaceHtmlEntities(req.body.message)
                );
                // , 7000});
                // waMessageQueue.add({
